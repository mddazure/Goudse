...
vnetgwtunnelip1=$(az network vnet-gateway show -n g-branch-2-gw -g Goudse --query "bgpSettings.bgpPeeringAddresses[0].tunnelIpAddresses[0]" --output tsv)
echo "VNET GW Tunnel address #1:" $vnetgwtunnelip1
vnetgwtunnelip2=$(az network vnet-gateway show -n g-branch-2-gw -g Goudse --query "bgpSettings.bgpPeeringAddresses[1].tunnelIpAddresses[0]" --output tsv)
echo "VNET GW Tunnel address #2:" $vnetgwtunnelip2
vnetgwbgpip1=$(az network vnet-gateway show -n g-branch-2-gw -g Goudse --query "bgpSettings.bgpPeeringAddresses[0].defaultBgpIpAddresses"  --output tsv)
echo "VNET GW BGP address:" $vnetgwbgpip1
vnetgwbgpip2=$(az network vnet-gateway show -n g-branch-2-gw -g Goudse --query "bgpSettings.bgpPeeringAddresses[1].defaultBgpIpAddresses"  --output tsv)
echo "VNET GW BGP address:" $vnetgwbgpip2
vnetgwasn=$(az network vnet-gateway show -n g-branch-2-gw -g Goudse --query "bgpSettings.asn" --output tsv)
echo "VNET GW BGP ASN:" $vnetgwasn
sharedkey="goudse"

echo "# VWAN: Create remote site"
az network vpn-site create --name branch2 -g Goudse --ip-address $vnetgwtunnelip1 --virtual-wan vwan-0 --location swedencentral --device-model VNETGW --device-vendor Azure --asn $vnetgwasn --bgp-peering-address $vnetgwbgpip1 --link-speed 100 --with-link true
...
az network vpn-gateway connection create --gateway-name 29f39501b78545a29959db3200deca31-swedencentral-gw --name branch-2-connection  -g Goudse --remote-vpn-site branch2 --shared-key $sharedkey --enable-bgp true --no-wait
...
az network vpn-connection create -n branch-2-hub-0 --vnet-gateway1 g-branch-2-gw -g Goudse --local-gateway2 lng1-1 -l swedencentral --shared-key $sharedkey --enable-bgp
az network vpn-connection create -n branch-2-hub-0-2 --vnet-gateway1 g-branch-2-gw -g Goudse --local-gateway2 lng1-2 -l swedencentral --shared-key $sharedkey --enable-bgp
